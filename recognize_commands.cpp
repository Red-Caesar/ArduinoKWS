...
RecognizeCommands::RecognizeCommands(int32_t average_window_duration_ms,
    ...
    : average_window_duration_ms_(average_window_duration_ms),
      ...
      previous_results_() {
  previous_top_label_ = kCategoryLabels[10];  // silence
...
TfLiteStatus RecognizeCommands::ProcessLatestResults(
  ...
  // MicroPrintf("Result: %d", latest_results->data.int8[0]);
  ...
  for (int offset = 0; offset < previous_results_.size(); ++offset) {
    ...
    for (int i = 0; i < kCategoryCount; ++i) {
      ...
      // MicroPrintf("Result: %d, index: %d", scores[i], i);
    }
  }
  for (int i = 0; i < kCategoryCount; ++i) {
  ...
  if ((previous_top_label_ == kCategoryLabels[10]) ||
      (previous_top_label_time_ == std::numeric_limits<int32_t>::min())) {
  ...
  *score = current_top_score;
  ...
  MicroPrintf("Scores: down %d go %d left %d no %d off %d on %d right %d stop %d up %d yes %d silence %d unknown %d  %s -> %s", 
                average_scores[0],
                average_scores[1], average_scores[2], average_scores[3], average_scores[4],
                average_scores[5], average_scores[6], average_scores[7], average_scores[8],
                average_scores[9], average_scores[10], average_scores[11],
                previous_top_label_, current_top_label);
  return kTfLiteOk;
}
